    ...
    return (
        <div className="dark w-full h-full bg-[#17120f] rounded-lg flex flex-col font-sans overflow-hidden">
            ...
            <div className="h-9 bg-[#2a211b] flex items-center gap-2 px-3 border-b border-[rgba(255,255,255,0.06)] shrink-0">
                <button className={`p-1 rounded transition-colors ${currentBatch ? 'text-text-secondary hover:text-white hover:bg-[#2a211b]' : 'text-[#6f6257] cursor-default'}`} onClick={goToRoot} disabled={!currentBatch}>
                ...
                <div className="flex gap-0.5">
                    <button className={`p-1 rounded transition-colors ${viewMode === 'grid' ? 'text-white bg-[#2a211b]' : 'text-text-muted hover:text-white'}`} onClick={() => setViewMode('grid')}>
                        <Grid3x3 className="w-3.5 h-3.5" />
                    </button>
                    <button className={`p-1 rounded transition-colors ${viewMode === 'list' ? 'text-white bg-[#2a211b]' : 'text-text-muted hover:text-white'}`} onClick={() => setViewMode('list')}>
                ...
                {!currentBatch ? (
                    <div className="grid grid-cols-[repeat(auto-fill,minmax(100px,1fr))] gap-2">
                        {EXPLORER_BATCHES.map(b => (
                            <button key={b.id} className="flex flex-col items-center gap-1 p-3 rounded-lg hover:bg-[rgba(255,255,255,0.04)] transition-colors cursor-pointer group" onClick={() => openFolder(b)}>
                    ...
                    <div className="grid grid-cols-[repeat(auto-fill,minmax(80px,1fr))] gap-2">
                        {visibleFiles.map((f, i) => (
                            <div key={i} className="flex flex-col items-center gap-1 cursor-pointer group">
                                <div className="w-full aspect-[4/3] rounded group-hover:ring-2 ring-[#ff7a45] transition-all" style={{ background: thumbGradient(f.name) }} />
                                <span className="text-[10px] text-text-secondary truncate w-full text-center">{f.name}</span>
                            </div>
                        ))}
                        {remaining > 0 && (
                            <button className="flex flex-col items-center justify-center gap-1 p-2 rounded-lg border border-dashed border-[rgba(255,255,255,0.1)] text-text-muted hover:text-white hover:border-[#ff7a45] transition-colors cursor-pointer aspect-[4/3]" onClick={() => setFilesShown(p => p + EXPLORER_FILES_PER_PAGE)}>
                    ...
                    <div className="space-y-0">
                        ...
                        {remaining > 0 && (
                            <button className="w-full py-2 mt-1 text-[11px] text-text-muted hover:text-white hover:bg-[rgba(255,255,255,0.04)] rounded transition-colors cursor-pointer" onClick={() => setFilesShown(p => p + EXPLORER_FILES_PER_PAGE)}>
            ...
            <div className="h-7 bg-[#2a211b] flex items-center justify-between px-3 border-t border-[rgba(255,255,255,0.06)] shrink-0">
                <span className="text-[10px] text-text-muted">{currentBatch ? `${currentBatch.count} items` : `${EXPLORER_BATCHES.length} folders`}</span>
                <span className="text-[10px] text-text-muted">{currentBatch ? `${(currentBatch.count * 3.8).toFixed(1)} MB` : '20.0 GB'}</span>
            </div>
        </div>
    );
};
...
const DropZonePanel = () => (
  <div className="dark w-full bg-[#17120f] rounded-2xl border border-border-subtle p-8 flex items-center justify-center aspect-[4/3]">
    <div className="w-full max-w-sm aspect-[4/3] border-2 border-dashed border-[#6f6257] rounded-xl flex items-center justify-center bg-[#211a15]">
      <div className="text-center px-4">
        <FolderOpen className="w-14 h-14 text-[#a89a8c] mx-auto mb-3" strokeWidth={1.5} />
        <p className="text-text-primary text-lg font-semibold">Drop a folder here</p>
        <p className="text-text-muted text-sm font-mono mt-1">12,480 photos · scanned locally</p>
      </div>
    </div>
  </div>
)
...
const CompletePanel = () => (
  <div className="dark w-full bg-[#211a15] rounded-2xl border border-border-subtle p-8 flex flex-col items-center justify-center aspect-[4/3]">
    <CheckCircle className="w-12 h-12 text-[#10b981] mb-2" />
    <h3 className="text-text-primary text-lg font-bold">Batching complete</h3>
    <p className="text-text-muted text-sm mt-1 mb-5">Created <span className="font-mono text-text-secondary">25</span> batch folders in <span className="font-mono text-text-secondary">3.1s</span>.</p>
    <div className="flex flex-wrap gap-2 justify-center">
      <button className="flex items-center gap-1.5 px-3 py-2 bg-[#2a211b] text-[#a89a8c] hover:text-white rounded-md text-xs font-semibold transition-colors"><Undo2 className="w-3.5 h-3.5" /> Undo</button>
      <button className="flex items-center gap-1.5 px-3 py-2 bg-[#2a211b] text-[#a89a8c] hover:text-white rounded-md text-xs font-semibold transition-colors"><History className="w-3.5 h-3.5" /> History</button>
      <button className="flex items-center gap-1.5 px-3 py-2 bg-[#2a211b] text-[#a89a8c] hover:text-white rounded-md text-xs font-semibold transition-colors"><FolderOpen className="w-3.5 h-3.5" /> Open</button>
      <button className="flex items-center gap-1.5 px-4 py-2 bg-gradient-to-r from-[#ff7a45] to-[#c7472a] text-white rounded-md text-xs font-semibold shadow-lg shadow-primary/20"><RotateCcw className="w-3.5 h-3.5" /> Process another</button>
    </div>
  </div>
)
  ...
  <div className="border-t border-border-subtle">
    <div className="mx-auto max-w-7xl px-6 lg:px-8 py-20 sm:py-28">
      <div className="grid lg:grid-cols-2 gap-12 lg:gap-16 items-center">
        ...
        >
          <span className="text-xs font-mono uppercase tracking-widest text-accent">{eyebrow}</span>
...
const ScrollToTop = () => {
  ...
  return (
    <button
      ...
      className={`fixed bottom-6 right-6 z-50 w-11 h-11 rounded-full bg-primary hover:bg-primary-hover text-white shadow-lg shadow-primary/25 hover:shadow-primary/40 flex items-center justify-center transition-all duration-300 cursor-pointer ${visible ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-4 pointer-events-none'}`}
    >
    ...
    <div className="relative bg-bg-main">
      ...
      <div className="relative z-10 border-t border-border-subtle">
        <p className="mx-auto max-w-3xl px-6 py-10 text-center text-lg text-text-secondary">
          <span className="font-mono text-text-muted">1–2 hours</span> of sorting by hand becomes <span className="font-mono text-accent">a few seconds</span>.
        </p>
      </div>
      ...
      <div className="relative z-10 border-t border-border-subtle">
        <div className="mx-auto max-w-5xl px-6 py-12 flex flex-wrap items-center justify-center gap-x-8 gap-y-3">
          <span className="text-xs font-mono uppercase tracking-widest text-text-muted">Made for</span>
      ...
      <div className="relative z-10 border-t border-border-subtle">
        <div className="mx-auto max-w-3xl px-6 py-28 text-center">
          <p className="text-sm font-semibold text-accent mb-4">Get started</p>
          ...
          <div className="mt-10 flex flex-col sm:flex-row items-center justify-center gap-4">
            <a href="/demo" target="_blank" rel="noopener noreferrer" className="inline-flex items-center gap-2.5 px-8 py-4 rounded-xl bg-primary hover:bg-primary-hover text-white text-base font-semibold shadow-lg shadow-primary/25 hover:-translate-y-0.5 transition-all">
