...
export default function AdminCoupons() {
  ...
  // ── Fetch all coupons (direct table access — RLS admin policies) ───────────
  const fetchCoupons = useCallback(async () => {
    try {
      ...
      if (listError) throw new Error('Failed to fetch coupons')

      // Compute per-coupon usage_count client-side (mirrors what the server did)
      const { data: usageRows, error: usageError } = await supabase
        .from('referral_usage')
        .select('coupon_id')
      if (usageError) throw new Error('Failed to fetch coupons')

      const counts = {}
      for (const row of usageRows || []) {
        counts[row.coupon_id] = (counts[row.coupon_id] || 0) + 1
      }
      setCoupons((data || []).map(c => ({ ...c, usage_count: counts[c.id] || 0 })))
    } catch (err) {
    ...
    try {
      ...
      if (insertError) {
        throw new Error(insertError.code === '23505'
          ...
          : (insertError.message || 'Failed to create coupon'))
      }
  ...
  const handleUpdate = async (id) => {
    try {
      ...
      if (fields.expires_at) fields.expires_at = new Date(fields.expires_at).toISOString()
      const { data, error: updateError } = await supabase
        ...
        .select()
        .single()
      if (updateError) throw new Error(updateError.message || 'Failed to update coupon')
      setSuccess(`Coupon "${data.code}" updated!`)
      setEditingId(null)
      setEditForm({})
      fetchCoupons()
    } catch (err) {
  ...
  const handleToggleActive = async (coupon) => {
    try {
      ...
      const { data, error: toggleError } = await supabase
        ...
        .select()
        .single()
      if (toggleError) throw new Error(toggleError.message || 'Failed to toggle coupon')
      setSuccess(`Coupon "${coupon.code}" ${data.is_active ? 'activated' : 'deactivated'}`)
      fetchCoupons()
    } catch (err) {
      setError(err.message)
    }
  ...
  const handleDelete = async (coupon) => {
    ...
    try {
      setError('')
      setSuccess('')
      // Soft-delete rule (was server-side): coupons with usage history are
      ...
      const { data: usageRows, error: usageError } = await supabase
        ...
        .limit(1)
      if (usageError) throw new Error(usageError.message || 'Failed to delete coupon')
      ...
      if (usageRows && usageRows.length > 0) {
        const { error: softError } = await supabase
          ...
          .eq('id', coupon.id)
        if (softError) throw new Error(softError.message || 'Failed to delete coupon')
        setSuccess(`Coupon "${coupon.code}" deactivated (has usage history)`)
      } else {
        const { error: delError } = await supabase
          .from('referral_coupons')
          .delete()
          .eq('id', coupon.id)
        if (delError) throw new Error(delError.message || 'Failed to delete coupon')
  ...
  const handleViewUsage = async (coupon) => {
    try {
      ...
      if (usageError) throw new Error(usageError.message || 'Failed to fetch usage')
      setUsageData(data || [])
    } catch (err) {
      setError(err.message)
      setUsageCoupon(null)
