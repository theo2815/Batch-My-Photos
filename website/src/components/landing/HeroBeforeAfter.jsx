...
import { Folder, Play, ArrowDown } from 'lucide-react'
...
// Deterministic pseudo-random in [0,1) so the pile scatters across the panel.
const rand = (n) => { const x = Math.sin(n * 12.9898) * 43758.5453; return x - Math.floor(x) }

// BEFORE: a dense pile of unsorted shots scattered across the panel, clipped inside it.
// Values are rounded so server and client emit identical style strings — Math.sin
// is not bit-identical across JS engines, and the raw floats caused a hydration mismatch.
const PILE = Array.from({ length: 40 }, (_, i) => ({
  name: `IMG_${String(4100 + ((i * 53) % 900) + i).padStart(4, '0')}.jpg`,
  rot: +(rand(i + 1) * 44 - 22).toFixed(2),
  top: +(1 + rand(i + 7) * 74).toFixed(2),
  left: +(-6 + rand(i + 13) * 94).toFixed(2),
  z: Math.floor(rand(i + 19) * 40),
}))
...
/**
 * Asymmetric editorial hero: the message sits left, the before/after proof sits
 * right (a messy pile of unsorted photos settling into clean numbered folders).
 * One gentle reveal on load; reduced-motion renders it static.
 */
      ...
      <div className="relative z-10 mx-auto w-full max-w-7xl px-6 lg:px-8 py-24">
        <div className="grid items-center gap-12 lg:gap-16 lg:grid-cols-[1.05fr_0.95fr]">
          ...
          <div className="text-center lg:text-left">
            ...
            <h1 className="font-hero text-[2.5rem] leading-[0.95] sm:text-6xl lg:text-[3.9rem] text-text-primary text-balance">
              Drop a messy folder.{' '}
              <span className="text-primary">Get clean, numbered batches</span>{' '}
              in seconds.
            </h1>

            <p className="mt-6 text-lg text-text-secondary max-w-xl mx-auto lg:mx-0 text-pretty">
            ...
            <div className="mt-9 flex flex-wrap items-center justify-center lg:justify-start gap-x-6 gap-y-3">
              <a
                ...
                className="group inline-flex items-center gap-2.5 px-7 py-3.5 rounded-full bg-primary hover:bg-primary-hover text-white text-base font-bold transition-[background-color,transform] duration-200 hover:-translate-y-0.5 active:scale-[0.98]"
              >
          ...
          <div className="relative mx-auto w-full max-w-md lg:max-w-none">
            ...
            {/* Connector */}
            <div className="relative z-10 flex items-center justify-center gap-2.5 py-2">
              <MotionDiv
                initial={reduce ? { opacity: 1, scale: 1 } : { opacity: 0, scale: 0.6 }}
                animate={{ opacity: 1, scale: 1 }}
                transition={{ delay: reduce ? 0 : 0.5, duration: 0.4, ease: 'backOut' }}
                className="w-11 h-11 rounded-full bg-primary text-white border-4 border-bg-main shadow-card flex items-center justify-center shrink-0"
              >
                <ArrowDown className="w-5 h-5" />
              </MotionDiv>
              <span className="text-xs font-mono text-text-muted tnum">sorted in 3.1s</span>
            </div>
            ...
            >
              ...
              <div className="h-44 sm:h-52 p-3 grid grid-cols-2 sm:grid-cols-3 gap-2 content-start">
                {FOLDERS.map((f) => (
